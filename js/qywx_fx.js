var companyId = getQueryString('companyId') || "216549";
...
var app = new Vue({
	...
	methods:{
			...
			$.ajax({
				...
				success: function(res){
					...
					wx.config({
							...
							jsApiList: ['onMenuShareAppMessage'] // 必填，需要使用的JS接口列表，凡是要调用的接口都需要传进来
					});
		...
		queryByExternalUserId: function(id){
			...
			$.ajax({
				type: "post",
				url:"http://39.107.74.133:9180/measured/queryByExternalUserId",
				dataType:"Json",
				data:{
					externalUserId: id,
				},
				success: function(res){
					...
					//alert(res.result);
					if(res.code == 200){
						location.href = "http://kj101-ysc.jiankangzhan.com/aijiankangshi/qw_historylist.html?customId="+res.result+"&companyId="+companyId
					}else{
