	...
	methods: {
		...
		azySendReportSaas: function() {
			...
			$.ajax({
				...
				error: function() {
					console.log('azySendReportSaas error')
				}
			});
		},
		//查看报告来源
		getReportSource: function() {
			...
			$.ajax({
				...
				error: function() {
					console.log('getReportSource error')
				}
			});
		},
		isUsableByCustomerIdAndNeId: function() { //判断优惠卷是否可用,返回有效期最短的优惠券
			...
			$.ajax({
				...
				error: function() {
					console.log('isUsableByCustomerIdAndNeId error')
				}
			});
		},
		//更新SaaS用户最近一份报告ID
		...
		gotoPay: function(e) { //点击支付
			...
			$.ajax({
				...
				error: function() {
					alert('payOrder error');
					$('#goWxPay').attr("disabled", false);
				}
			});
		},
				...
				success: function(data) {
					if (data.code == 200) {
						if (reportType == 121 || reportType == 122 || reportType == 12001 ||
							reportType == 123) {
							location.href = 'report120.html?reportId=' + reportId + '&userId=' +
								userId + '&reportType=' + reportType + '&openId=' + openId +
								'&saasId=' + saasId
						} else if (reportType == 501 || reportType == 502 || reportType ==
							5021 || reportType == 505 || reportType == 503) {
							location.href = 'report500.html?reportId=' + reportId + '&userId=' +
								userId + '&reportType=' + reportType + '&openId=' + openId +
								'&saasId=' + saasId
						} else {
							location.href = 'report' + reportType + '.html?reportId=' +
								reportId + '&userId=' + userId + '&reportType=' + reportType +
								'&openId=' + openId + '&saasId=' + saasId
						}
					}
				}
			});
			...
			$.ajax({
				...
				success: function(res) {
					if (res.code == 200) {
						...
						var reportUrl = '?reportId=' + report + '&userId=' + res.data
							.customerId + '&openId=' + open + "&reportType=" + reportType +
							'&saasId=' + saas + '&source=' + source + '&reportSource=' +
							reportSource;
						if (visible == 0) {
							$('.v_overlay').css({
								"visibility": "visible",
				...
				error: function() {
					alert('analysisReportFace error')
				}
			});
		},
