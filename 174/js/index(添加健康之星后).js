...
$.ajax({
		...
		if(data.code == 200){
			...
			success : function(indexData) {
				...
				if(indexData.code == 201){
					...
					if(paymentType == 1){
						window.location.href="payforOld.html?reportId=" + myReportId + '&openId=' + myopenId + "&sameUser=" + sameUser;
					}else if(paymentType == 2){
						window.location.href="payfor.html?reportId=" + myReportId + '&openId=' + myopenId + "&sameUser=" + sameUser;
					};
				...
				success: function(jkzxData){
					if(jkzxData.code == 200){
						if(jkzxData.data == true){
							//已参加健康之星活动，查数据最优报告
							$.ajax({
								url: dataUrl + "/api/v1/activity/findActivityByUserReport",
								type: "POST",
								dataType: 'json',
								data: {
									openId : myopenId
								...
								success: function(bestData){
									if(bestData.code == 200){
										for(var n=0;n<bestData.data.length;n++){
											var oindex = n+1;
											var bgcon = document.createElement('div');
											bgcon.className = 'bgcon'
											bgcon.innerHTML = '<span>'+ oindex +'</span><span>'+ bestData.data[n].totalScore +'</span><span>'+ bestData.data[n].createTimeStr +'</span>'
											$('.jkzx_list').append(bgcon);
										}
									}	
								},
								error: function(xhr,status){
									console.log('健康之星查寻最优报告失败'+ xhr.status + " " + xhr.statusText)
								}
							});
							$('.jkzx_box').css("display","block");
						}else if(jkzxData.data == false){
							$('#jkzx_go').css("display","block");
							//$('#jkzx_go').attr("href",dataUrl + "/wxUser/wxUserReport?jumpUrl=jkzx");
						}
						$('#jkzx_go').attr("href",dataUrl + "/wxUser/wxUserReport?jumpUrl=jkzx");
					}else{
	...
	error: function(xhr,status){
		alert("解析-错误提示： " + xhr.status + " " + xhr.statusText);
		$('.load-overlay').css("display","none");
		$('#error_con').css("display","block");
	}
});
